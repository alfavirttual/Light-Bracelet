...
unsigned char c = 0; /* биты
                         ...
                         */
unsigned char h = 0; // счетчик нажатий 
...
ISR(INT0_vect){
  ...
  if (config & 0b00010000){ // Если разблокированно 
    ...
    if((TCNT0 > 200) & ((hold_tap != 0b00010010) || (hold_tap != 0b00100110))) {// Смотрим входит ли в период короткого нажатия
          hold_tap = 0b00000000;
    }
    if ((hold_tap == 0b00000010) || (hold_tap == 0b00010110)) hold_tap = 0;
...
void wdt_ini(){
  WDTCR |= (1 << WDTIE); // Разрешение прерывания после переполнения таймера
  WDTCR &= ~(1 << WDE); // Режим перываний без сброса
  WDTCR |= (1 << WDP3) | (1 << WDP0); //Период срабатывания 8с.
}
