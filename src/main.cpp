...
#define LED_LIGHT PB5 // Пин к которому подключен основной светодиод
...
#define ADC_PIN PB2 // Пин(АЦП) к которому подключена батарея 
...
#define WDT_TIME WDTO_4S //Время по истечению которого сработает прерывание wdt таймера 

// #define LOW_BATTERY_IMPULSE_NUM 8 // Число импульсов, сигнализирующих о низком заряде батаери 
// #define LOW_BATTTER_IMPULSE_TIME 240 // Длительность импульсов сигнализирующих о ником заряде батареи 
...
/* void wdt_enable(){
  cli(); // Запрещаю глобальные прерывания
  MCUSR &= ~(1 << WDRF); // Сбрасываем флаг прерывания, если он был установлен 
  WDTCR |= (1 << WDCE) | (1 << WDE); // Активирование записи
  WDTCR = (1 << WDTIE) | (1 << WDP3); // Настройка на срабатывание через 4с в режиме Interrupt
...
void wdt_disable(){
  cli(); // Запрещаю глобальные прерывания
  MCUSR &= ~(1 << WDRF); // Сбрасываем флаг прерывания, если он был установлен 
  WDTCR |= (1 << WDCE) | (1 << WDE); // Разрешаю запись в регистр WDTCR
  WDTCR = 0x00; // Выключаю таймер 
  sei(); // Разрешаю глобальные прерывания
}
 */
...
ISR(WDT_vect){
  PORTB &= ~(1 << LED_GREEN) & ~(1 << LED_YELLOW) & ~(1 << LED_RED) & ~(1 << LED_LIGHT);
  wdt_disable();
  MCUCR &= ~(1 << ISC00); // Прерывание при низком уровне
  sei();
  sleep_enable();
  sleep_cpu();
  sleep_disable();
  cli();
  config = 0;
  wdt_enable(WWDT_TIME);
  WDTCR |= (1 << WDTIE);
  MCUCR |= (1 << ISC00); // Прерывание при смене логического уровня
  GIFR = (1 << INTF0); // Очищаем регистр флагов внешних прерываний
  ADCSRA |= (1 << ADIF); // Очищаем флаг прерывания АЦП
  sei();
}

void port_ini(){
  DDRB &= ~(1 << BUTTON_PIN) & ~(1 << ADC_PIN); // Настраиваю пин INT0 и пин АЦП на вход
...
void adc_ini(){
  ...
  #if (ADC_PIN == PB2)
  ADMUX &= ~(1 << MUX1); // Выбор пина ADC ADC_PIN
  ADMUX |= (1 << MUX0);
  #elif (ADC_PIN == PB3)
  ADMUX |= (1 << MUX0) | (1 << MUX1); // Выбор пина ADC ADC_PIN
  #elif (ADC_PIN == PB4)
  ADMUX &= ~(1 << MUX0); // Выбор пина ADC ADC_PIN
  ADMUX |= (1 << MUX1);
  #elif (ADC_PIN == PB5)
  ADMUX &= ~(1 << MUX0) & ~(1 << MUX1); // Выбор пина ADC ADC_PIN
  #endif
